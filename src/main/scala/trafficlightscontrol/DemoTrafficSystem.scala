...
class DemoTrafficSystem(period: FiniteDuration = 10.seconds) extends Actor with ActorLogging {
  ...
  val lights1: Map[String, ActorRef] = (0 to 3) map { c => ("" + c -> context.actorOf(Props(classOf[TrafficLightFSM], "" + c, RedLight, period / 10))) } toMap
  val group1: ActorRef = context.actorOf(Props(classOf[OnlyOneIsGreenSwitch], lights1, period))
